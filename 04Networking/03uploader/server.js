...
const port = 8787;
const hostName = "localhost"; // "::1" is IPv6 loopback (like 127.0.0.1 for IPv4)
...
const server = net.createServer(async (socket) => {
  ...
  let isFileHandlerCreated = false; // Track if the file stream is ini  tialized
    ...
    try {
      ...
      if (!isFileHandlerCreated) {
        ...
        if (!parsedString.includes(txtSeperator) ) {
          socket.write("ERROR: Invalid header. Expected 'fileName:' followed by newline\n", () => {
            socket.end(); // Close connection after error
          });
          return;
        }

        // Extract filename from header only (until newline)
        const headerEnd = parsedString.indexOf("\n");
        const fileName = parsedString
          .substring(parsedString.indexOf(txtSeperator) + txtSeperator.length, headerEnd)
          .trim();
        ...
        // Step 1b: Write any remaining data after the header to the file
        const remainingData = data.subarray(headerEnd + 1);
        if (remainingData.length > 0) {
          const canWrite = fileStream.write(remainingData);
          if (!canWrite) {
            socket.pause();
  ...
  socket.on("end", async () => {
    ...
    console.log("File transfer finished for one client.");
  });
